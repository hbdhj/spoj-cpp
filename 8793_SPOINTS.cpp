...
inline bool inConvexPoly(point *C, int nc, const point &p) 
{
	...
	int d1, d2, d3;
	if((d1 = triArea2(C[0], C[st], p)) < 0) 
        return false;
	if((d2 = triArea2(C[st], C[en], p)) < 0) 
        return false;
	if((d3 = triArea2(C[en], C[0], p)) < 0) 
        return false;
	if(!d1 && onsegment(C[0], C[st], p)) 
        return true;
	if(!d2 && onsegment(C[st], C[en], p)) 
        return true;
	if(!d3 && onsegment(C[en], C[0], p)) 
        return true;
...
int main() 
{
	...
	while(scanf("%d %d", &n1, &n2) == 2 && (n1 | n2))
    {
		...
		if(n1 >= 3) 
            convexHull(bp, n1, bc, c1);
		else 
        { 
            memcpy(bc, bp, sizeof(point) * n1); 
            c1 = n1; 
        }
		if(n2 >= 3) 
            convexHull(wp, n2, wc, c2);
		else 
        { 
            memcpy(wc, wp, sizeof(point) * n2); 
            c2 = n2; 
        }
		if(checkIntersect(bc, c1, wc, c2))
            puts("NO");
		else if(checkContained(bc, c1, wc, c2))
            puts("NO");
		else if(checkContained(wc, c2, bc, c1))
            puts("NO");
		else 
            puts("YES");
	}
