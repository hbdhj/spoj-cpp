...
int main() 
{
	...
	while(test--) 
    {
		...
		res[0] = calc(p[1], p[2], p[3], p[4]); 
        i = 0;
		res[1] = calc(p[0], p[2], p[3], p[4]); 
        if(res[1] > res[i]) 
            i = 1;
		res[2] = calc(p[0], p[1], p[3], p[4]);
        if(res[2] > res[i]) 
            i = 2;
		res[3] = calc(p[0], p[1], p[2], p[4]); 
        if(res[3] > res[i]) 
            i = 3;
		res[4] = calc(p[0], p[1], p[2], p[3]); 
        if(res[4] > res[i]) 
            i = 4;
		printf("%d\n", i + 1);
	}
